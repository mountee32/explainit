...
// if (isset(getallheaders()['Authorization']) && getallheaders()['Authorization'] === 'Bearer ' . $secret_token) {
    ...
    if (!empty($data['id'])) {
        $stmt = $conn->prepare("DELETE FROM qa_categories WHERE id = :id");
        $stmt->bindParam(':id', $data['id']);

        if ($stmt->execute()) {
            http_response_code(200);
            echo json_encode(array("message" => "Category deleted successfully."));
            file_put_contents($log_file, "{$time_stamp} - delete api - Category deleted successfully " . json_encode($data) . "\n", FILE_APPEND);
        } else {
            http_response_code(503);
            echo json_encode(array("message" => "Unable to delete category."));
            file_put_contents($log_file, "{$time_stamp} - delete api - Unable to delete Category " . json_encode($data) . "\n", FILE_APPEND);
        }
    } else {
        http_response_code(400);
        echo json_encode(array("message" => "Unable to delete category. ID is missing."));
        file_put_contents($log_file, "{$time_stamp} - delete api - Unable to delete category. ID is missing" . json_encode($data) . "\n", FILE_APPEND);
    }
// } else {
